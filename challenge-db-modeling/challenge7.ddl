...
-- 타임라인미디어
DROP TABLE IF EXISTS TMLME RESTRICT;
...
CREATE TABLE TRN (
    ...
    TRNTIME VARCHAR(255) NOT NULL COMMENT '상담시간', -- 상담시간
...
CREATE TABLE PMEMB (
    ...
    PTYPE  INTEGER      NOT NULL COMMENT '유형' -- 유형
)
COMMENT '프로그램참여자';
...
CREATE TABLE MSG (
    ...
    MSTITL VARCHAR(255) NULL     COMMENT '제목', -- 제목
...
-- 타임라인미디어
CREATE TABLE TMLME (
    TMLMENO  INTEGER      NOT NULL COMMENT '타임라인미디어번호', -- 타임라인미디어번호
    TMLNO    INTEGER      NOT NULL COMMENT '타임라인 번호', -- 타임라인 번호
    TMLMPATH VARCHAR(255) NOT NULL COMMENT '사진경로' -- 사진경로
)
COMMENT '타임라인미디어';

-- 타임라인미디어
ALTER TABLE TMLME
    ADD CONSTRAINT PK_TMLME -- 타임라인미디어 기본키
        PRIMARY KEY (
            TMLMENO -- 타임라인미디어번호
        );

ALTER TABLE TMLME
    MODIFY COLUMN TMLMENO INTEGER NOT NULL AUTO_INCREMENT COMMENT '타임라인미디어번호';
...
-- 프로그램참여자
ALTER TABLE PMEMB
    ADD CONSTRAINT FK_USER_TO_PMEMB -- 사용자 -> 프로그램참여자
        FOREIGN KEY (
            UNO -- 사용자번호
        )
        REFERENCES USER ( -- 사용자
...
-- 운동계획서
ALTER TABLE PLAN
    ADD CONSTRAINT FK_PROG_TO_PLAN -- 프로그램 -> 운동계획서
        FOREIGN KEY (
            PNO -- 프로그램번호
        )
        REFERENCES PROG ( -- 프로그램
...
-- 타임라인미디어
ALTER TABLE TMLME
    ADD CONSTRAINT FK_TML_TO_TMLME -- 타임라인 -> 타임라인미디어
        FOREIGN KEY (
            TMLNO -- 타임라인 번호
        )
        REFERENCES TML ( -- 타임라인
